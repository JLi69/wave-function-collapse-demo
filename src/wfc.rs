...
use crate::{ImageData, wrap_value};
use std::{collections::{HashMap, HashSet, VecDeque}, ops::Range};
...
impl WFCParameters {
    ...
    pub fn from_image_data(data: &ImageData, tile_sz: isize) -> Self {
        ...
        for y in 0..(data.height as isize) {
            for x in 0..(data.width as isize) {
                ...
                add_rule(&mut rules[id], UP, get_id(
                    &grid_ids,
                    x,
                    y - 1,
                    data.width,
                ...
                add_rule(&mut rules[id], RIGHT, get_id(
                    &grid_ids,
                    x + 1,
                    y,
                    data.width,
                ...
                add_rule(&mut rules[id], DOWN, get_id(
                    &grid_ids,
                    x,
                    y + 1,
                    data.width,
                ...
                add_rule(&mut rules[id], LEFT, get_id(
                    &grid_ids,
                    x - 1,
                    y,
                    data.width,
    ...
    pub fn generate_grid(&self, w: usize, h: usize) -> ImageData {
        ...
        let mut lowest_entropy_tiles = lowest_entropy(&superpositions, &not_collapsed, self.wfc_tiles.len());
        ...
        while lowest_entropy_tiles.len() > 0 {
            ...
            not_collapsed = not_collapsed.iter()
                .filter(|index| superpositions[**index].len() > 1)
                .map(|index| *index)
                .collect();
            lowest_entropy_tiles = lowest_entropy(&superpositions, &not_collapsed, self.wfc_tiles.len());
        }
        ...
        for y in 0..h {
            for x in 0..w {
                ...
                grid[index] = 
                    self.wfc_tiles[superpositions[index][0]][tile_index];
            }
        }
...
fn lowest_entropy(superpositions: &Vec<Vec<usize>>, not_collapsed: &Vec<usize>, max_entropy: usize) -> Vec<usize> {
